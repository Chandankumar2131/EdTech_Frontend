  ...
  useEffect(() => {
    ...
    const fetchCourseDetailsData = async () => {
      try {
        ...
        console.log("course details res cc: ", res)
        setResponse(res)
      } catch (error) {
    ...
    return (
      <div className={`mt-24 p-5 flex flex-col justify-center gap-4`}>
        <div className="flex flex-col sm:flex-col-reverse gap-4">
          <p className="h-44 sm:h-24 sm:w-[60%] rounded-xl bg-gray-300 animate-pulse"></p>
          <p className="h-9 sm:w-[39%] rounded-xl bg-gray-300 animate-pulse"></p>
        </div>

        <p className="h-4 w-[55%] lg:w-[25%] rounded-xl bg-gray-300 animate-pulse"></p>
        <p className="h-4 w-[75%] lg:w-[30%] rounded-xl bg-gray-300 animate-pulse"></p>
        <p className="h-4 w-[35%] lg:w-[10%] rounded-xl bg-gray-300 animate-pulse"></p>

        {/* Floating Courses Card */}
        <div className="right-[1.5rem] top-[20%] hidden lg:block lg:absolute min-h-[450px] w-1/3 max-w-[410px] 
            translate-y-24 md:translate-y-0 rounded-xl bg-gray-300 animate-pulse">
        </div>

        <p className="mt-24 h-60 lg:w-[60%] rounded-xl bg-gray-300 animate-pulse"></p>
      </div>
    )
  }
        ...
        <div className="mx-auto box-content px-4 lg:w-[1260px] 2xl:relative">
          <div className="mx-auto grid min-h-[450px] max-w-maxContentTab justify-items-center py-8 lg:mx-0 lg:justify-items-start lg:py-0 xl:max-w-[810px]">
            ...
            <div className="mb-5 lg:mt-10 lg:mb-0 z-[100]" onClick={() => navigate(-1)}>
              <GiReturnArrow className="w-10 h-10 text-yellow-200 hover:text-yellow-100 cursor-pointer" />
            </div>
            ...
            <div className="flex w-full flex-col gap-4 border-y border-y-gray-500 py-4 lg:hidden">
              <p className="space-x-3 pb-4 text-3xl font-semibold text-white">Rs. {price}</p>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {
                tag && tag.map((item, ind) => (
                  <p key={ind} className="bg-yellow-100 p-[2px] text-black rounded-full text-center font-semibold">
                    {item}
                  </p>
                ))
              }
              ...
              <div className="flex items-center gap-4 py-4">
                ...
                <div>
                  <p className="text-lg capitalize flex items-center gap-2 font-semibold">{`${instructor.firstName} ${instructor.lastName}`}
                    <span><MdOutlineVerified className='w-5 h-5 text-blue-400' /></span>
                  </p>
                  <p className="text-gray-400">{instructor?.additionalDetails?.about}</p>
                </div>
              </div>
            </div>
          </div>
